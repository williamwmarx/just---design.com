...
const Sheets = {
  getData(filename, sheet_id, page, column_count) {
    return fetch(
      ...
      .then(data => {
        ...
        fs.writeFile(
          path.join(__dirname, "../static/json/" + filename),
...
for (let key of Object.keys(data_to_load)) {
  ...
  Sheets.getData(filename, sheet_id, data_to_load[key]["page"], data_to_load[key]["column_count"])
}
