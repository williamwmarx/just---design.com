        ...
        <CardStack ref={this.cardstackRef}>
          {this.state.sources.map((source_data, card_num) => {
            ...
            let source = {
              ...
              "creators": source_data[4]?.split("`").map((k, i) => [k, source_data[5]?.split("`")[i]])
            }

            // Search query string
            let search_string = [].concat(...Object.values(source)).filter(String).join(" ").toLowerCase()
              ...
              return (
                <Card key={`card_${card_num}`}>
                  <Card.Header>
                    <h4>{source["title"]}</h4>
                    <h5>
                      {source["creators"] && source["creators"].map((creator, idx) => {
                        return (
                  ...
                  <Card.Body>
                    <p>{source["summary"]}</p>
                    <Card.Button href={source["source_url"]}>View this resource <Emoji emoji="⚒️"/></Card.Button>
                  </Card.Body>
                </Card>
              )
            }
